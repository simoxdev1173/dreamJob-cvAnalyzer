...
export default function HeroCarouselArc() {
    ...
    useEffect(() => {
        ...
        autoplayTimeline.current = gsap.timeline({
                repeat: -1
            })
            .to(ring, {
                rotationY: '+=360',
    ...
    useEffect(() => {
        ...
        const dragEnd = () => {
            ...
            // --- THE FIX ---
            // Kill the old timeline and create a new one starting from the current rotation
            autoplayTimeline.current?.kill();
            autoplayTimeline.current = gsap.timeline({
                    repeat: -1
                })
                .to(ring, {
                    rotationY: '+=360', // This continues the rotation from the current spot
                    duration: 30,
                    ease: 'none',
                    onUpdate: applyArcEffect,
                });
            ...
            <div className="relative w-[300px] h-[400px]" style={{ perspective: '2000px' }}>
                <div ref={ringRef} className="absolute w-full h-full" style={{ transformStyle: 'preserve-3d' }}>
                    {[
                        ...
                        <div
                            key={i}
                            className="absolute w-[300px] h-[400px] bg-cover bg-center bg-no-repeat rounded-lg"
                        />
                    ))}
                </div>
            </div>
